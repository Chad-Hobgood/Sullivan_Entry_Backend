...
The entry workflow expects a tab named `EV Design studio` and writes analytics to `Dashboard_Data_Link`. Its installable edit trigger reads only student-ID columns from the private lookup workbook and writes status values, not roster details, into the entry workbook.
